The ongoing implementation is performed in both Intel OpenMP runtime (iOMP) Version 20160322 and GNU Compiler Runtime (GOMP) version 6.1.0. The implementations for 
...
environment setting for {\sf OMP\_WAIT\_POLICY} and the cooresponding ICV, the use of turnaround and throughput mode for OpenMP execution, the 
use of {\sf KMP\_BLOCKTIME} and kmp\_set\_blocktime for setting the waiting time before putting a thread to sleep~\cite{iccmanual}, 
