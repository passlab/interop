...
\multirow{2}{*}{PASSIVE}& SPIN\_YIELD & User-level busy wait &  while (!finished()) sched\_yield() ;  \\ \cline{2-4}
                      & SLEEP       & Thread sleeps. Others wake it up. & mutex\_wait(); mutex\_wake();\\ \hline
\end{tabular}
}  
...
For GOMP, implementation is very similar. GOMP uses a spin count to control how long a thread waits actively
before waiting passively. On Linux systems, busy wait is implemented by running a loop checking the waiting condition
...
The implementation of {\sf omp\_quiesce(omp\_thread\_state\_KILL)} is a wrapper of {\sf gomp\_free\_thread} which frees a thread pool and releases its threads. 
