...
In this section, we study the interoperability challenges, and motivate our proposal. 
...
interoperability. 
\subsection{Three Types of Use Cases} % of OpenMP Interoperability}
...
In first two types of use cases for intra-node interoperabity, 
...
Most OpenMP implementations, however, ignore their existence % user threads or other parallel runtime instance 
...
Thirdly, the {\sf OMP\_THREAD\_LIMIT} environment, {\sf thread-limit-var} ICV and the {\sf omp\_get\_thread\_limit} getter give users an option to  
 set the maximum number of OpenMP threads to use in a contention group when the program starts, addressing oversubscriptions issues in certain
 ...
 its actualy effects in real applications. 
